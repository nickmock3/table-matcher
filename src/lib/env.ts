...
const serverEnvSchema = z.object({
  ...
  GOOGLE_REDIRECT_URI_DEMO: z.string().url().optional(),
  GOOGLE_REDIRECT_URI_PROD: z.string().url(),
  APP_BASE_URL_LOCAL: z.string().url().optional(),
  APP_BASE_URL_DEV: z.string().url(),
  APP_BASE_URL_DEMO: z.string().url().optional(),
  APP_BASE_URL_PROD: z.string().url(),
  APP_ENV: z.enum(["local", "dev", "demo", "prod", "e2e"]).default("dev"),
});
...
const getRawServerEnv = () => ({
  ...
  GOOGLE_REDIRECT_URI_DEMO: process.env.GOOGLE_REDIRECT_URI_DEMO,
  GOOGLE_REDIRECT_URI_PROD: process.env.GOOGLE_REDIRECT_URI_PROD,
  APP_BASE_URL_LOCAL: process.env.APP_BASE_URL_LOCAL,
  APP_BASE_URL_DEV: process.env.APP_BASE_URL_DEV,
  APP_BASE_URL_DEMO: process.env.APP_BASE_URL_DEMO,
