...
- 公開画面の反映方式は、次フェーズのDB連携時に「ページ表示時のサーバー再取得（SSR）」を基本方針とする。
- 反映データは `seat_status_updates` の最新有効レコード（`expiresAt > now`）を `createdAt DESC, id DESC` で解決する。

## 変更履歴
- 2026-02-16: 初版作成
- 2026-02-16: URL・公開画面反映方式の方針を確定
